...
fn main() -> ! {
    ...
    if let Some(p) = pac::Peripherals::take() {
        ...
        // Send address
        p.smb5.smbn_txf_ctl().modify(|_, w| unsafe{w.tx_thr().bits(0)});
        ...
        // Send address
        p.smb5.smbn_rxf_ctl().modify(|_, w| unsafe {w.rx_thr().bits(1).thr_rxie().clear_bit().last_pec().set_bit()});
